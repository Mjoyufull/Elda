...
- Latest delta (2026-06-11): `elda su` and `elda dsu` are root commands for updating the running Elda executable from `https://github.com/Mjoyufull/Elda`; `su` builds `main`, `dsu` builds `dev`, both use the locked release build for `elda-cli`, verify the produced binary, and replace the current executable while preserving its mode and owner. `--dry-run` reports the selected branch and destination, and `--offline` fails closed.
