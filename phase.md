...
The phases here are implementation order only. They do not redefine the product downward. Elda is still the full system described in `SPEC.md`: native package definitions, binary-first delivery, transactional install state, CI publication, interbuild support, interepo translation, and migration/adoption.

### 1.1 Process Status Percentages

Implementation-process percentages for the current market/UX continuation track:

- **Pre-Phase-11+ focus:** **~97%** for the named native CLI/UX slice (runtime-backed surfaces plus listed post-prephase polish). May 2026 operator-frame pass closed `::` frames, build passthrough, review gate dedup including unchanged interbuild review fast paths, bulk snapshot parity, and semantic color; remaining gaps: upgrade/rm/sync unified transaction frames, bootstrap privilege deferral, and MiB preflight polish. Phase-11 interepo/coexistence remains separate.
- **Post-prephase polish (same milestone, not Phase 11):** **~98%** for ledger-listed items; dedicated nested inner-build frame deferred in favor of live stdio passthrough.
...
- Latest delta (2026-05-28): upstream pkgit's current C implementation was audited against Elda and the replacement gap is now explicit instead of generic: Elda covers current pkgit add/install/remove/list/search/update/source-build/tag-ref workflows, while direct `bldit.lua` as a first-class schema and pkgit's raw copy/symlink heuristic install mode remain the pkgit-specific gaps. Hosted `ci pr` auth now supports `auth = "bearer"` as well as token/ssh/none, and system activation snapshots now support direct `btrfs subvolume snapshot -r` in addition to `snapper`; blanket post-build activation prompts are no longer tracked as a hard requirement.
- Latest delta (2026-08-21): the local-artifact path is closed end to end rather than only metadata-smoked. Local-only recipes retain a canonical source path; archive staging honors surveyed root stripping; content magic wins over a lying suffix; tar+bzip2 and ZIP join tar/gzip/xz/zstd, ELF, and supported AppImage inputs; ELF/AppImage architecture is recorded; AppImages require a verified SquashFS or DwarFS payload; DwarFS payloads pass through without unsupported desktop extraction; bounded member/path/size limits fail closed; `artifact-survey.json` preserves the exact inventory; human and JSON review surfaces carry the survey; and a disposable-root regression generates metadata, installs the archive, and executes the installed launcher. The added production modules were split below the repository's 350-line soft ceiling. Verification passed with `cargo fmt --all`, `cargo test --workspace`, `cargo build --workspace`, strict all-target/all-feature Clippy, and `git diff --check`.
- Latest delta (2026-08-18): `elda a <path>` now handles a local artifact file directly. Pointing Elda at a downloaded release archive identifies the container by content magic rather than extension, surveys it read-only (no extraction, no execution), classifies every member, infers package name and version from the archive root or file name, computes the payload digest, and writes a digest-pinned binary-lane recipe. Previously that path treated an archive as a git source tree and produced a recipe named after the whole file including its extension. `--from <url>` records the download origin so the package can be re-fetched and upgraded; without it an interactive run asks once and a non-interactive run records the package as local-only and says so in the generated recipe. A launcher is recorded only when the archive holds exactly one executable candidate, and cross-platform members such as `.exe` / `.dll` / `.ps1` are reported as dropped instead of being staged.
- Latest delta (2026-08-14, contract): the CLI visual contract is frozen. Frame grammar, row types, key-column alignment, the glyph set, the palette and provenance badges, density rules, gate placement, live-progress rules, failure shape, and human-surface snapshot expectations now have a single owning contract ledger, and `SPEC.md` §16.1 was reconciled to it — the `Target`/`Resolution`/`Plan`/`Progress`/`Result` section order is retired in favour of one compact gate frame plus a delta-only result. Earlier CLI redesign and install-UX notes are demoted to design rationale rather than shape authority. Runtime fix in the same pass: the `[F]` (foreign repository, translated) and `[V]` (vendor/ad-hoc link) provenance badges no longer share a colour, and `[?]` renders muted. Separately, build-dependency cleanup is specified in `SPEC.md` §16.2 but is not implemented anywhere in the runtime — only reported — so the reported set now at least excludes packages that were already installed, and the choice between implementing cleanup and amending §16.2 is recorded as an open contract question.
- Latest delta (2026-08): read-only query commands no longer escalate privilege or mutate the root. `elda ls`, `list`, `search`, `info`, `files`, `verify`, `reverify`, `why`, `rdeps`, `versions`, `diff`, `check`, and their namespaced read-only counterparts now run through a shared `command_class` classifier (unknown paths default to mutating), and the state database gained a read-only handle that opens `SQLITE_OPEN_READ_ONLY`, answers an absent database from an in-memory schema, and never creates the layout or takes the mutation lock — previously `elda ls` on a live host demanded a sudo password and then created directories. Query paths also tolerate an absent world/journal/current-state file instead of erroring, and cache-policy sizing walks to the nearest existing ancestor rather than requiring the cache directory to exist. Install gate stacking is reduced: the temporary build-dependency prompt and the release-trust-key prompt no longer fire after the operator already accepted the transaction plan; both are now rows in that plan (`build deps::`, `trust keys::` with full un-truncated key fingerprints on continuation lines), `[install].remove_build_deps` implements a documented cleanup opt-out that had never been built, and `elda u` no longer double-prompts through both the dispatch gate and its own plan gate. `sync` no longer prompts at all. Frame `key:: value` rows are column-aligned with a width cap, the colouriser preserves that padding, borders render dim, `ls` drops the duplicate `area status` banner in favour of the table plus a muted count, and the interactive search picker reprompts inline on a bad selection instead of aborting the search. Locale detection now follows POSIX precedence (`LC_ALL` → `LC_CTYPE` → `LANG`) and treats an empty variable as unset, which previously downgraded every frame to ASCII on hosts with `LANG=`. AppImage payload detection now covers DwarFS as well as SquashFS, accepts unmarked `uruntime`/RunImage-family binaries when a payload magic verifies at a computed offset, rejects Type 1 with a specific error, and reports the container format through `elda appimage inspect`.
- Latest delta (2026-06-02): raw-link metadata no longer silently reuses an older local recipe with the same inferred name, and failed remote Git probes stop instead of scaffolding pretend ad hoc metadata; official `archlinux.org/packages/...` pages normalize to the matching Arch packaging Git source for bounded PKGBUILD review; generated checksum-backed release assets default plain installs to the binary lane unless the operator selected a source option, raw binary assets use asset-derived `rename` instead of guessed archive `binary`, and `--replace` URL installs force the explicit target back through build/stage even when the package version is unchanged; generated HTTP(S) URL metadata now falls back to the source URL for `upstream` when parser-backed homepage metadata is absent; verified binary tar payloads can now infer the launcher only when the archive contains exactly one executable candidate, otherwise they fail closed until `source.binary` is explicit; AUR `.SRCINFO` is preferred for expanded metadata and native-architecture SHA-256-backed `-bin` archives become binary lanes; generated ad hoc git metadata now records static build intent for supported repo markers (`build.system` plus intended `build.bins`) so the final launcher set is visible during metadata review; unversioned GNU-host `glibc`, `libgcc`, and `libstdc++` plus musl-host `musl` dependencies are treated as external platform capabilities instead of impossible Elda recipe lookups; the second post-metadata activation prompt now honestly advertises `[Y/n]` because edits belong to the metadata review gate.
- Latest delta (2026-05-27): the deterministic human-surface snapshot harness is wired into `elda-core` tests, covers 59 required CLI families, and now checks ASCII/NO_COLOR output, raw-JSON fallback prevention, and legacy first-line `area: ok` preambles. Shared framed renderers no longer prepend duplicate `area: status` plus summary before command-specific frames; legacy scan headers use non-colon `area ok` wording while structured frames carry the operative facts.
- Latest delta (2026-05-24): install, bulk snapshot, interbuild, and metadata review frames now use compact `key:: value` rows with plain keys and selective value emphasis (`render_style.rs`); dispatch confirmation persists across privilege re-exec and `elda a`/`elda add` no longer self-confirm before their metadata or bulk snapshot review gates; metadata/interbuild/install review gates use a single frame-footer `[Y/n/e]` with no duplicate prompt after pager; live cargo/cmake/meson/make/ctest builds inherit child stdio when streaming (one Elda header line, native tool ANSI/progress like paru); session logs resolve under the invoking operator home through sudo (`ELDA_OPERATOR_HOME`); snapshot-imported interbuild recipes reuse local template trees instead of re-cloning; mutating human commands restore the terminal and exit 130 on Ctrl+C; live progress uses green success checkmarks and orange build-step labels. Prior delta (2026-05-18): `elda review ls`, `elda review info`, `elda review forget`, and `elda review diff` now expose recorded source-definition review stamps; interbuild review opens generated metadata in `ELDA_PAGER`/`PAGER`/`less` before the `[Y/n/e]` gate when content changed; install dry-run preflight now also reports review-memory status, pending configuration files, shared-path policy, and privilege posture; global `--no-stream` now disables live progress streaming for human and JSON runs, `[display].tree_chars` now forces Unicode, ASCII, or automatic tree-frame detection, and privilege re-exec now prints a framed provider/policy handoff before sudo/doas/run0/su; `elda sync <remote...>` now targets named remotes; `rc show`, `rc diff`, and `rc publish-ready` expose recipe source metadata, local-vs-synced drift, and publish-readiness blockers; `ls` has operator filters; `files search`, `config pending`, `config diff`, `config apply`, `config keep`, and `trigger ls/info` now expose installed path search, conffile queue state/resolution, and system-trigger state in runtime output; generated-metadata and interbuild review gates now persist content-addressed review stamps so unchanged recipe content does not re-prompt; generated metadata acceptance now stops at a second install confirmation frame before build/stage/activation; install dry-runs include a preflight block with known managed-byte and filesystem-space data; `elda doctor` now reports bootstrap paths, remotes, backend health, advisories, and release-readiness flags; install/upgrade human plans name replacement targets; Cargo source builds resolve case-mismatched binary targets from Cargo metadata; interbuild and bulk snapshot review frames now expose parser/source/snapshot context more explicitly; `[git].allowed_protocols` now fails closed on disallowed git transports across source, interbuild, remote recipe, preview, and interemote sync paths; `elda git releases --tag <ref>` now filters release inspection to a chosen tag; generated git-release binary recipes now preserve metadata from detected interbuild/native source definitions and emit source+binary lanes when a checksum-backed release asset is available; metadata import paths now preserve existing `pkg.lua` / `build.lua` / patch metadata unless `--replace` is explicit; the public docs/examples pass now makes `/etc/elda` config shape, dynamic interemote remotes, native forge hosting, the quick README, USAGE, fixtures, and the `man/elda.1` operator page line up with that runtime surface.
...
- the `elda-core` runtime has been refactored out of the old giant dispatcher/test-file shape into focused modules (`app_install`, rendering, progress, tests split by area), so the codebase now tracks the workspace structure standards materially better while preserving current behavior
- the current standards pass also split the old `elda-install`, `elda-db` store, `elda-cli` CLI/help and privilege path, `elda-types` package/version parsing, `elda-recipe` parser/vendor/check/import flow, `elda-repo` snapshot layer, `elda-core` runtime slabs including `app_install/dependency/*`, `app_install/binary_source.rs`, `app_repo/*`, the new `cache_policy/*` cleanup path, the new `app_ci/*` runtime, and the expanded `app_profile/*` selection flow into smaller units; the workspace is currently green, and the latest standards scan found 57 Rust files over the `350` soft-limit ceiling across older runtime modules, integration tests, and snapshot harnesses
...
- Implementing the full Elda contract from `SPEC.md`.
...
- A line-by-line port of upstream pkgit's C code.
...
**Rollout Strategy:** Elda is built in parallel to upstream `pkgit`, which is reference material rather than a local runtime dependency. The early bring-up path is crate skeleton -> DB and resolver -> staged payloads -> prefix backend -> Linux system backend -> CI/native publish -> interbuild -> interepo -> migration. We do not call Elda the replacement for `pkgit` until the transaction engine and Linux activation backend are real.
...
Upstream pkgit behavior and C source are reference material, fixture input, and importer source.
They are not the product codebase Elda will keep extending.
...
2. `phase.md`
3. `USAGE.md`
4. Upstream pkgit documentation and C source as reference and import material only
...
- **Scope:** Create the Rust workspace and crate skeleton from `SPEC.md` section 19, wire the canonical CLI namespaces, and capture `pkgit` fixture inputs for behaviors we are preserving or importing.
- **Hardness:** H1
- **Dependencies:** None
- **Done-when:** `cargo check` passes for the workspace skeleton, `elda --help` exposes the canonical namespaces from the spec, and fixture samples exist for `pkgdeps`, `bldit`, direct git installs, and repo search/list flows from upstream pkgit.
...
| Phase 9: Native CI and Binary Publishing | Completed | Current slice complete | 2026-05-28 | `ci sub/run/status/pr/retry/logs`, `ci batch new/add/push`, `forge search/browse`, `qa lint/build/smoke/stack/repro/diff`, and `daemon run` now execute a real local/filesystem-backed package-definition and publish workspace, including copied artifacts, `lock-v1.json.zst`, signed local index output, per-payload `.minisig` / `.spdx.json` / `.attestation.json` sidecars, indexed `sbom_url` / `attestation_url` metadata, configurable submission remote/base-branch targeting, local scheduler queue processing through bare `ci run`, retry/state tracking, compare/PR-style URL derivation from recognizable forge remotes with configured base-branch awareness, and token/bearer-auth hosted review creation through `ci pr` with per-remote override resolution for GitHub/GitLab/Gitea-style APIs; the current native CI slice is still intentionally local-first and file-backed, but the documented Phase 9 proof points are now met |
| Phase 10: Git-Mode Interbuilds | Completed | Current slice complete | 2026-06-02 | Bounded parser-backed source-lane installs are live for `nix_flake`, `gentoo_overlay`, `aur_pkgbuild`, and `xbps_template`; official Arch package pages normalize to packaging Git sources for bounded PKGBUILD review; `elda a` / `elda add` performs metadata-only raw-link resolution with config-ordered source strategy detection, field-confidence reporting, actionable `--source-option` / `--strategy` selection, ad hoc git add/install/update `--to-branch` / `--to-tag` / `--to-rev` metadata pinning, pinned git-ref keep-installed dry-run reporting, config-backed git tag policy defaults, and generated AUR/XBPS metadata filling; failed remote Git probes and same-name local metadata reuse fail closed; AUR `.SRCINFO` supplies expanded metadata and native-architecture SHA-256-backed `-bin` archive lanes; Gentoo/AUR/XBPS reports include accepted phase command lines, and the human interbuild review gate now includes parser-specific metadata/dependency-family context; AUR/XBPS fail closed on unsupported shell expansion, mismatched source/checksum counts, or mismatched AUR arch-specific source/checksum counts; bounded host runtime capabilities do not become impossible local recipe dependencies; the binary lane accepts provider-aware `release_asset` for GitHub/GitLab/Gitea/Forgejo/SourceHut/direct manifests; generated checksum-backed release archives record launcher names and default plain installs to binary unless a source option was explicit; human `list-options` installs can prompt for a source option on TTYs; ad hoc git packages can source-ref downgrade with `downgrade --to-tag/--to-rev`; `elda git tags` / `elda versions` list local/remote tags with normalized version confidence and can optionally join matching releases through `--with-releases`; `elda git releases` inspects GitHub/GitLab/Gitea/Forgejo/SourceHut/direct-manifest release assets with OS/arch/libc/format compatibility scoring and self-hosted forge host preservation; raw release links can convert to pinned `github_release` or `release_asset` metadata when a digest or checksum sidecar is available while keeping source fallback selected when checksum data is missing; the Lua parser properly handles generated empty tables without false-positive validation errors; GPKG binary fast-path and release signature sidecar metadata validation are wired for the Phase 10 slice; install-time release signature trust is gated by configured trust keys. |
...
- Pre-Phase-11+ native CLI slice remains closed for named runtime surfaces, while the UX polish tracker remains roughly **97%/98%** to avoid claiming full CLI completeness.
...
- Updated supporting interepo notes to match those decisions.

### v0.1.45 - 2026-05-18

- Split native forge hosting into `eldaforgehosting/` with per-topic and per-platform how-tos (source-only, binhost, cache, trust, interemotes, recipe-to-Git paths, GitHub/GitLab/Gitea-Forgejo/SourceHut/static HTTP, and AUR/binhost/full-forge/LAN-mirror/staging patterns).
- Reconciled git-source UX notes against landed pre-Phase-11+ CLI (`--pick-tag`, dispatch `[Y/n/e]`, `rc format`/`normalize`, `forge fork`).

### v0.1.44 - 2026-05-16

- Updated the public operator documentation set around the current pre-Phase-11+ runtime: `README.md` is now a concise project entrypoint, `USAGE.md` covers current command families, `eldaforgehosting/` has explicit source-only, binary, cache, GitHub/GitLab/Gitea, and interemote hosting examples, and `man/elda.1` now exists as a local man page source.
...
- Extended the recipe flag system inside Phase 10 source-lane work as outlined in
  the flag contract: `pkg.lua` now declares `flags_descriptions`,
...
- Config update: configuration examples, `su/config.toml`, and config fixtures now carry the runtime-backed `[metadata].link_strategy_priority` order used by `elda a` / `elda add`; config parsing has a `MetadataConfig` shape plus fixture-load regression coverage.
...
- Added a thin `elda` CLI binary that exposes the canonical root commands and namespace tree from `SPEC.md`.
...
- Created `phase.md` as the implementation-order companion to `SPEC.md`.
