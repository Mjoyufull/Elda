...
- **Post-prephase polish (same milestone, not Phase 11):** **~98%** for ledger-listed items; dedicated §13.6 nested inner-build frame deferred in favor of live stdio passthrough.
...
The upstream pkgit behavior and C source are reference material, fixture input, and importer source.
...
- Pre-Phase-11+ native CLI slice remains closed for named runtime surfaces, while the UX polish ledger stays at roughly **97%/98%** to avoid claiming full CLI completeness.
...
- Refreshed `su/config.toml`, `examples/config/`, and `fixtures/config/` so the sample `/etc/elda` shape reflects current runtime fields, metadata replacement policy, git transport policy, dynamic interemote remotes, source-capable `packages_url` remotes, cache documents, and extension documents.
...
- Extended the recipe flag system inside Phase 10 source-lane work as outlined in
  `pkg.lua` now declares `flags_descriptions`,
