...
| Phase 5: Build/staging/payloads | Completed | Current slice complete | [x] stage + manifest + payload; [x] git/archive/github_release paths; [x] local artifact content survey plus digest-pinned generated install path for tar/ZIP/ELF/supported AppImage inputs; [x] synced source builds materialize pinned `packages/<pkgname>/` trees from `packages_url` + `repo_commit`; [x] build metadata now captures typed `provider_assets`; [~] source-kind parity is a later source-model/runtime gap, not a Phase 5 blocker |
...
| Phase 10: Git-mode interbuilds | Completed | Current slice complete | [x] bounded `nix_flake`, `gentoo_overlay`, `aur_pkgbuild`, and `xbps_template` source installs; [x] official Arch package-page to packaging-Git normalization; [x] bounded host runtime dependency satisfaction for generated foreign metadata; [x] expanded AUR `.SRCINFO` metadata and SHA-256-backed native `-bin` archive lanes; [x] parser metadata reports and interbuild review gates; [x] config-ordered add/link strategy plus explicit `--source-option` / `--strategy`; [x] failed Git probes and stale same-name local metadata reuse fail closed for URL installs; [x] generated checksum-backed release assets default to binary, raw release executables use asset-derived `rename`, and `--replace` URL installs force restage for unchanged versions; [x] verified binary tar payloads can infer one unambiguous executable launcher and otherwise fail closed; [x] ad hoc git ref selection and source-ref downgrade; [x] provider-aware `release_asset` binary lane through GitHub/GitLab/Gitea/Forgejo/SourceHut/direct manifests; [x] release signature sidecar metadata and safe-field validation; [x] install-time release signature trust gates declared sidecars through `[trust].release_keys`; [x] `elda git releases --tag <ref>`; [x] `[git].allowed_protocols` fail-closed clone gating; [x] GPKG binary fast path; [x] installed file search, config queue resolution, trigger inspection, and dynamic remote/interemote UX; [~] broader Nix evaluation and full PKGBUILD/XBPS shell semantics remain Phase-11+/post-prephase depth, not current-slice blockers |
...
- [x] Current slice - Generated source metadata records detected `build.system` and `build.bins` when static repo markers make output intent clear
- [x] Current slice - Binary tar lanes infer a launcher only from a single verified executable candidate, with explicit `binary` required for ambiguity
...
- [x] Current slice - `ELDA-03` Resolved for the current pre-Phase-11+ slice: bounded parser-backed families, source-option selection, ad hoc git ref selectors, provider-aware release assets, read-only multi-provider release inspection, release signature sidecar metadata validation plus install-time trust gating, GPKG fast path, config queue resolution, and inspection UX are runtime-backed; broader full-foreign semantics move to Phase-11+/post-prephase work
...
- [x] Current slice - upstream pkgit parity audit is reconciled, hosted `ci pr` auth supports bearer tokens, and system activation snapshots support direct `btrfs` alongside `snapper`
...
- [x] Current slice - Validate release signature fields for empty/traversal values and gate declared sidecar installs through `[trust].release_keys`
