...
| --- | --- | --- |
| `(root)` | `a`, `add`, `i`, `ig`, `ib`, `rm`, `u`, `su`, `dsu`, `sync`, `ls`, `search`, `info`, `files`, `verify`, `reverify`, `why`, `rdeps`, `pin`, `unpin`, `hold`, `unhold`, `adopt`, `downgrade`, `diff`, `check`, `recover`, `rollback`, `fix-triggers`, `autoremove` | Core package-manager operations |
...
- `elda su` self-updates the running `elda` executable from `https://github.com/Mjoyufull/Elda` on `main`; `elda dsu` does the same from `dev`. Both clone the selected branch, run `cargo build --release --locked -p elda-cli`, verify the built binary, and replace the current executable while preserving its mode and owner. `--dry-run` reports the selected branch and destination without cloning or replacing; `--offline` fails closed.
